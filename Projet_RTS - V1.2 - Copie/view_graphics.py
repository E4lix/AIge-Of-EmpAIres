...
# Boucle de jeu principale : Gère les evenements, met à jour l'écran et rafraichit le graphisme
def game_loop(units, buildings, game_map, ai, delay=0.1):
    running = True
    clock = pygame.time.Clock()
    view_x, view_y = 0, 0  # emplacement de la caméra au début du jeu
    max_width = screen_width // tile_size
    max_height = screen_height // tile_size
    last_update_time = time.time()

    while running:
        current_time = time.time() # mesure du temps qui passe
        ...
        keys = pygame.key.get_pressed()
        if keys[pygame.K_F12]:  # changer pour le mode temrinal
            pygame.quit()
            sys.exit()  # quite le mode graphique
